...
* The proportion of male victims peaks between the ages of 17-24, especially for Black vicitims
...
username <- "go.dave"
apikey <- "lVPzJ5yJGE2KESUj2n9F"

Sys.setenv("plotly_username" = username)
Sys.setenv("plotly_api_key" = apikey)

api_create(p11, filename = "London homicides, 2008-2019", sharing = "public")
...
ggplot(LondonHomicideRate, aes(x= as.factor(Year), y=Homicide_Rate)) +
...
Defining people by ethnicity is highly subjective and can be deceptive so great care is needed here. The original mmap database created by Iain Ager inlcuded this variable and we have retained this variable and his ratings for the cases in 2019 and 2020.
...
The volume of young Black homicide victims in London is worryingly high during the period observed. Between the ages of 11 and 20 more than two-thirds of homicide victims in London were Black, despite accounting for just fewer than 1 in 5 residents within this age group. White are overcrepresented as homicide victims in London when considering the 50 and over age groups.
...
mmap12 <- mmap11 %>% arrange(desc(mmap11$homicide_rate2))

colnames(mmap12) <- c("Age_Group", "Homicide_Rate")

View(mmap12)

p20 <- ggplot(mmap12, aes(x = Age_Group, y = Homicide_Rate)) +
...
p30 <- ggplot(mmap10, aes(x= reorder(NAME, homicide_rate), y=homicide_rate)) +
geom_bar(stat="identity", fill="navyblue", color = "white")+
  geom_label(size = 1, aes(label = homicide_rate, hjust = 0.2))+
  coord_flip()+
...
p31 <- ggplot(mmap10, aes(x= reorder(NAME, -homicide_rate), y=homicide_rate)) +
geom_bar(stat="identity", fill="navyblue", color = "white")+
...
+
ggsave("Percent_solved_AgeGroup.pdf", height = 4.5, width = 8)
...
# Read this shape file with readShapeSpatial 

London_Ward <- readShapeSpatial("London_Ward_CityMerged.shp")

# convert coordinates into lat long degrees
proj4string(London_Ward) <- CRS("+init=epsg:27700")
London_Ward.wgs84 <- spTransform(London_Ward, CRS("+init=epsg:4326"))

# check the class
class(London_Ward.wgs84)

names(London_Ward.wgs84)


# Convert London_Ward.wgs84 file into a data frame
library(broom)

London_Ward.wgs84.f <- tidy(London_Ward.wgs84, region = "DISTRICT")

# check levels for borough names
names(London_Ward.wgs84.f)

str(London_Ward.wgs84.f)

London_Ward.wgs84.f$id <- as.factor(London_Ward.wgs84.f$id)

str(London_Ward.wgs84.f)


# clean up the factor names within borough variable in London.shp.f to enable the bind with mmap12 (ensure they are exactly the same!)
library(dbplyr)

levels(London_Ward.wgs84.f$id)

London_Ward.wgs84.f2 <- London_Ward.wgs84.f %>% mutate(id = fct_recode(id, 
...
levels(London_Ward.wgs84.f2$id)

London_Ward.wgs84.f2$id <- as.character(London_Ward.wgs84.f2$id)

str(London_Ward.wgs84.f2)

mmap10 <- data.frame(mmap10)

str(mmap10)

# Merge data using  Ensure you use by="borough" to link the data frames by the unique borough names


merge.London <-left_join(London_Ward.wgs84.f2, mmap10,
                          by=c("id"="borough"), all.x=TRUE)

# order the final data frame to be used in the choropleth mapping.

choro.data.frame3<-merge.London[order(merge.London$order), ]

str(choro.data.frame3)


# Start with your first plot
p1 <- ggplot(data = choro.data.frame3,
             aes(x = long, y = lat, group = group,
                 fill = homicide_rate))
p1 + geom_polygon(color = "black", size = 0.25) +
  coord_map()

# add code to enhance and customise the data visualisation
p2 <- p1 + geom_polygon(color = "black", size = 0.25) +
  coord_map() +
  scale_fill_distiller(name = "rate per 100,000",
                        guide = "legend",
  ...
  labs(title="Average Annual Homicide Rate in London 2008:2019")

ggplotly(p2)

#3 create Leaflet interactive cloropeth chart for Homicide Rate
...
# You can plot the LGA polygons quickly using the leaflet function and addPolyons. You need to set a default view and zoom level.
p2 <- leaflet(London_Ward2.wgs84) %>%
  setView(lng = -0.118092 , lat = 51.509865, zoom = 10)
p2 %>% addPolygons()

# check class, names and levels
class(London_Ward2.wgs84)
names(London_Ward2.wgs84)
levels(London_Ward2.wgs84$NAME)


# Rename the borough variable in mmap10 to "NAME" to match the "SpatialPolygonsDataFrame" file London_Ward2.wgs84

names(mmap10)[names(mmap10) == "DISTRICT"] <- "NAME"
